...
**Label Detection:**
If the main notation line begins with a label (e.g., `Var 1:`, `Line 1)`), extract the label. Labels typically appear at the start of the line and are separated from the notation by whitespace. The extracted label must be stored under the `label` field in the `Main Line` section of the output.
...
```yaml
paragraph:
  main_pitch_system: [detected pitch system, e.g., "Sargam", "Western"]
  lines:
    - type: upper_annotation_line # Or lower_annotation_line, main_notation_line
      line_number_in_paragraph: 0 # 0-based index of this line within the original paragraph
      tokens:
        - type: space
          column: 0
          length: 2
    ...
    - type: main_notation_line
      ...
      label: "" # The extracted label, or an empty string if none
      pitch_system: [detected pitch system for this line]
      tokens:
        - type: note
          column: 0
          pitch: S
...
````
  ...
  * All tokens must include `column` (0-based index) and `line` (0-based index within the paragraph).
  ...
  lines:
    - type: main_notation_line
      ...
      tokens:
        - type: barline
          column: 0
          text: "|:" # Added text field for multi-char barlines
        - type: space
          column: 2
          length: 2
        - type: note
...
paragraph:
  ...
  lines:
    ...
    - type: main_notation_line
      ...
      tokens:
        - type: space # Added space for alignment if Label is present
          column: 0
          length: 8 # Length of "Line 1) "
        - type: barline
          column: 8
        - type: space
          column: 9
...
#### ✅ Notes:
  ...
  * **Crucially, the output must be valid YAML that can be directly parsed by a YAML parser.**
